...
def deckBuilder():
    ...
    wilds = [{"Kleur": "Wild","Waarde":"Null"},{"Kleur": "Wild","Waarde":"Draw Four"}]
...
while True:
    ...
    if inputHowManyPlayer > 2:
        print("Het spel heeft minimaal 2 spelers nodig!")
    else:
        break
...
while not isWinner(players) and not isWinnerScore(playerScores):
    ...
    if topCard["Waarde"] == "Draw Four":
        ...
        print("Er is een reverse gespeeld het rondje draait om!")
        if len(players) == 2:
            if Reverse == True:
                Reverse = False
                CurrentPlayer += 1
    ...
    while True:
        ...
        if canPlay(topCard, players[CurrentPlayer]):
            ...
            currentScore = ScoreConverter(playCard)
